...
Array.from(colors.values()).forEach((color) => {
  console.log(`<tr>
    <td>${color}</td>
    <td><div class="colorprev" style="background-color:var(--color-${color})" title="--color-${color}"></div></td>`);
  shades.forEach((shade) => {
    console.log(
      `<td><div class="colorprev" style="background-color:var(--color-${color}-${shade})" title="--color-${color}-${shade}"></div></td>`
    );
  });
  console.log('</tr>')
});
